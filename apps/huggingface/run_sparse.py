...
def import_graphdef(family,
    ...
    relay_file = "%s_%s_%d_%d_%s" % (family, name, batch_size, seq_len, relay_file)
    relay_params = "%s_%s_ %d_%d_%s" % (family, name, batch_size, seq_len, param_file)
    ...
    if os.path.exists(os.path.join(abs_path, relay_file)) and os.path.exists(
            os.path.join(abs_path, param_file)):
        with open(os.path.join(abs_path, relay_file), 'r') as fi:
            mod = tvm.ir.load_json(fi.read())
        with open(os.path.join(abs_path, param_file), 'rb') as fi:
...
@click.option('--platform', type=click.Choice(['cpu', 'gpu'], case_sensitive=False), default='cpu')
@click.option(
...
def run(family, name, batch_size, seq_len, platform, extract_llvm_options, run_sparse, bs_r,
        sparsity):
    mod, params, shape_dict = import_graphdef(family, name, batch_size=batch_size, seq_len=seq_len)
    ...
    if run_sparse and platform == 'cpu':
        print("Block Sparse Model {sparsity} {blocksize}x{blocksize}:".format(
            sparsity=sparsity, blocksize=bs_r))
